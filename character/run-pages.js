...
const graphqlFiles = files.filter(f => /^characters-page-\d+\.graphql$/.test(f));
...
for (const file of graphqlFiles) {
  ...
  const match = file.match(/^characters-page-(\d+)\.graphql$/);
  if (!match) continue;

  let Int = match[1];
  Int = Number(Int);
  const outputFile = `characters-page-${Int}-output.json`;
  ...
  try {
    ...
    console.log(`Processed ${file} → ${outputFile} (id=${Int})`);
  } catch (err) {
